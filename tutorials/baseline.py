...
import torch
# print("可用GPU数量：", torch.cuda.device_count())  # 应输出实际数量（如2）
# print("当前使用GPU：", torch.cuda.current_device())  # 应输出指定的编号（如1）
import time
import traceback
import warnings
...
def init_wandb():
    hyperparameter_defaults = dict(
        ...
        pastmse=False,
        replay=False,
        init_class = 12,             # few-shot场景下是7个初始类别, 原始是14
        filter_sample = False,     
        randomsplit = True,        # few-shot场景下需要False
        fewshot = None,                # None
        nlayers_cls=3,              # 3
        init_weight = False,
        ...
        adapter = False,
        freeze_all = True,
        loramoe = False,                      # 报错就是导入的Transformer模型没选对
        lora = False,
        batch_expert = False,
        num_of_expert = None,         # loramoe=False时为None
        num_of_gate = 4,           # =2 分开专家的时候
         ...
         "nlayers_cls" + str(config["nlayers_cls"]) + "_init_class" + str(config["init_class"]) + "_adapter_" + str(config["adapter"])\
              + "_freeze_all_" + str(config["freeze_all"]) +\
               "_fewshot_" + str(config["fewshot"]) +\
                ...
                # "重定义optimizer"
                # "_num_of_expert_" + str(config["num_of_expert"])
                # "_batch_expert_" + str(config["batch_expert"])
                # "_lora_" + str(config["lora"]) 
...
def prepare_data(sort_seq_batch, tokenized_train, tokenized_valid, train_batch_labels, valid_batch_labels, train_celltype_labels, \
    ...
    if input_gene_ids_train.shape[0]>=2000:
        ...
        input_gene_ids_train = input_gene_ids_train[perm][:2000]
        input_values_train = input_values_train[perm][:2000]
        target_values_train = target_values_train[perm][:2000]
        tensor_batch_labels_train = tensor_batch_labels_train[perm][:2000]
        tensor_celltype_labels_train = tensor_celltype_labels_train[perm][:2000]
...
class ContinualClassify():
    ...
    def prepare_model(self, num_batch_types, num_max_classes=2, modeldict_name="best_model.pt"):
        if self.config["loramoe"]:
            ...
            from model.loramodel import TransformerModel, AdversarialDiscriminator          # lora
        else:
            from scgpt.model import TransformerModel, AdversarialDiscriminator
        if self.config["load_model"] is not None:
        ...
        elif self.config["loramoe"] and self.config["DSBN"]:
            model = TransformerModel(
                len(self.vocab),
                embsize,
                nhead,
        ...
        for batch, batch_data in enumerate(itertools.islice(loader, num_batches)):
            ...
            with torch.cuda.amp.autocast(enabled=self.config["amp"]):
                if self.config["adapter"] or self.config["loramoe"] or self.config["lora"]:
                    ...
                    batch_id = torch.tensor(test_batch_idx) if self.config["DSBN"] else None,
                    CLS=True,
                    CCE=False,
                    MVC=self.config["MVC"],
            ...
            # logger.info(f"| epoch {epoch:3d} | {batch:3d}/{num_batches:3d} batches | "
            #         f"lr {lr} "
    ...
    def eval_testdata(self, adata_t, gene_ids, input_layer_key, test_batch_idx):
        self.model.eval()
    ...
    def build_fewshot_dataset(self, all_counts, celltypes_labels, batch_ids, shots_per_class=5):
        # np.random.seed(seed)
    ...
    def process_batch(self, adata_train, logger, save_dir, dataset_name, experiment_name, all_adata_test, all_batch_results, test_batch_idx):
        ...
        if test_batch_idx == 0:
            ...
            # self.model.cls_decoder.out_layer.out_features = len(self.old_model_annotation)
            if self.config["replay"]:
                example_bank_previous = torch.load(save_dir / "example_bank.pth")['example_bank']                         # 之前储存的样本
                # memory bank 标签和 batch
    ...
    def plot_gray_batch(self, adata, input_layer_key, gene_ids, test_batch_idx, save_dir, best_model = None, save_name="val", cell_type_map=None, legend_on=False):
        ...
        # Step 2: 合并所有向量
        X_all = np.concatenate([X_cells.detach().cpu().numpy()], axis=0)
        ...
        for test_batch_idx in range(len(highlight_batch_list)):
            ...
            if legend_on:
                ...
                plt.savefig(
                    str(save_dir) + "/" + save_name + f"gray_baseline_incremental_batch{test_batch_idx}_legend.png",
                    bbox_inches='tight',
                    edgecolor='black',
                    dpi=300
                )
                ...
                plt.savefig(
                    str(save_dir) + "/" + save_name + f"gray_baseline_incremental_batch{test_batch_idx}.png",
                    bbox_inches='tight',
                    edgecolor='black',
                    dpi=300
                )
    ...
    def subplot_clusters_prototypes(self, fig, adata, input_layer_key, gene_ids, test_batch_idx, save_dir, max_batch_idx, save_name="val", cell_type_map=None):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=self.config["amp"]):
            if self.config["adapter"] or self.config["loramoe"] or self.config["lora"]:
                cell_embeddings = self.model.encode_batch(
                    ...
                    src_key_padding_mask=src_key_padding_mask,
                    batch_size=self.config["batch_size"],
                    batch_labels= torch.from_numpy(np.full_like(batch_ids, fill_value=0)).long() if self.config["DSBN"] else None,
                    batch_id = torch.tensor(test_batch_idx) if self.config["DSBN"] else None,
                    time_step=0,
                    return_np=True,
                )
        ...
        # n_items = len(labels_leg)

        # # 判断 legend 列数
        # ncol = 1 if n_items <= 7 else 2

        # 把 legend 放在图外下方
        # ax1.legend(
        #     handles, labels_leg,
        #     loc="upper center",
        #     bbox_to_anchor=(0.5, -0.15),  # 图下方位置
        #     ncol=ncol,
        #     fontsize=20,
        #     frameon=False,
        #     markerscale=3 
    ...
    def evaluate_all(self, adata_train, save_dir, test_batch_idx, cell_type_map):
        ...
        if self.config["randomsplit"]:
            ...
            (
                ...
                train_indices, 
                val_indices
             ) = self.build_fewshot_dataset(all_counts, current_celltype_labels, batch_ids, shots_per_class=self.config["fewshot"])
            adata_train_split = adata_train[train_indices].copy()
            adata_valid_split = adata_train[val_indices].copy()     
...
def evaluate_predict(save_dir, cell_type_map):
    ...
    all_adata_list = []
    all_batch_results = {}
    fig, axs = plt.subplots(2, max_batch_idx, figsize=(5*max_batch_idx+2, 13), constrained_layout=True)
    ...
    for test_batch_idx in range(len(data_paths)):
        ...
        all_adata_list.append(adata_test)
        combined_adata_test = anndata.concat(
                    all_adata_list,
                    join="outer",
                    merge="unique",
        ...
        ############################### 画 celltype + batch 图 ############################
        sc1, sc2, celltype, batch = continual_classify.subplot_clusters_prototypes(fig, combined_adata_test, 
                            "X_binned", gene_ids, test_batch_idx, save_dir, max_batch_idx=max_batch_idx, save_name=f"trainval_batch_{test_batch_idx}_", cell_type_map=cell_type_map)
        ###################################################################################
    ...
    plt.tight_layout()
    # plt.legend()
    plt.savefig(str(save_dir) + "/" + f"trainval_batch_all_" + f"umap_cluster_prototype_legend.png", bbox_inches='tight', edgecolor='black',dpi=300)
    ...
    #     index_unique=None
    # )
    # continual_classify.predict(combined_adata_test, save_dir, 123456, cell_type_map)
...
def main():
    ...
    num_batch_types = 1
    if config["DSBN"]:
        num_batch_types = len(data_paths)
...
def predict(save_dir, cell_type_map, test_batch_list=None, modeldict_name="best_model_batch_5.pt"):
    ...
    num_batch_types = 1
    config["weight_dir"] = save_dir
   # 需要根据当前的数据集和采用的是online 还是 full batch来选择不同的预训练模型
    ...
    # continual_classify.plot_gray_batch(combined_adata_test, 
    #                          "X_binned", gene_ids, test_batch_idx, save_dir, 
    #                          save_name=f"batch_test_{test_batch_idx}",cell_type_map=cell_type_map,legend_on=True)
    # gene_ids = continual_classify.predict(
    #         combined_adata_test, save_dir, 100, cell_type_map
    #     )
    final_results, final_adata = continual_classify.eval_testdata(
        adata_t=combined_adata_test,
        gene_ids=gene_ids,
    ...
    #     bbox_inches='tight')
#############################################################
# def get_adata_test_by_batch(adata, test_batch_list):
#     """
#     根据给定的 test_batch_list，从 adata 中提取对应 batch_id 的测试数据子集。

#     参数：
#         adata: AnnData 对象，包含 .obs["batch_id"]
#         test_batch_list: 测试集所对应的 batch_id 列表（如 [6, 7, 8, 9]）

#     返回：
#         adata_test: 仅包含测试 batch 的 AnnData 子集
#     """
#     # 确保 batch_id 是整数类型
#     batch_ids = adata.obs["batch_id"].astype(int)
    
#     # 创建一个布尔索引，标记哪些样本属于测试 batch
#     test_mask = batch_ids.isin(test_batch_list)
...
# def predict_testall(save_dir, cell_type_map, test_batch_list):
#     config = init_wandb()
#     set_seed(config["seed"])
#     if config["dataset_name"] == "pancreas":
#         # data_paths = ["../data/PANCREAS/" + f"pancreas_batch{i}.h5ad" for i in range(6, 9)]  # 剩下三个batch作为测试集
#         data_paths = ["../data/PANCREAS/pancreas_data.h5ad"]
#     model_dir = Path(config["load_model"])
#     vocab_file = model_dir / "vocab.json"
#     vocab = GeneVocab.from_file(vocab_file)
#     special_tokens = ["<pad>", "<cls>", "<eoc>"]
...
#     # for test_batch_idx in range(len(data_paths)):
#     adata = sc.read_h5ad(data_paths[0])
#     adata_test = get_adata_test_by_batch(adata, test_batch_list)
#     gene_ids = continual_classify.predict(
#         adata_test, save_dir, cell_type_map
#     )
#         # all_adata_test.append(adata_test)
...
#     with open(str(save_dir) + "/"  "predict_testall_embeddings.json",
#               "w") as f:
#         json.dump(final_results, f, indent=4)
...
#     )
#     plt.savefig(
#         str(save_dir) + "/" + f"embeddings_batch_umap[cls]_batch_predict_testall.png",
#         bbox_inches='tight')
...
#     )
#     plt.savefig(
#         str(save_dir) + "/" + f"embeddings_celltype_umap[cls]_batch_predict_testall.png",
#         bbox_inches='tight')
if __name__ == "__main__":
    ...
    # save_dir = "../save/dev_pancreas-Jul23-16-51-56"
    # save_dir = "../save/dev_myeloid-Jul22-20-43-38"
    # save_dir = "../save/dev_pancreas-Jul29-11-45-31"           # loraonly 
    # save_dir = "../save/dev_pancreas-Jul29-17-19-20"
    # save_dir = "../save/dev_pancreas-Jul28-17-45-12"               # loramoe+warmcosine（expert=4) + 重新定义optimizer
    # save_dir = "../save/dev_pancreas-Jul28-10-41-23"             # loramoe+warmcosine（expert=2)
    # save_dir = "../save/dev_myeloid-Jul24-10-32-08"              # baseline
    # save_dir = "../save/dev_myeloid-Aug11-14-55-50"                # baseline + fewshot
    # save_dir = "../save/dev_myeloid-Jul24-10-32-08"          # myeloid 画图
    # save_dir = "../save/dev_pancreas-Jul01-16-49-34"
    # save_dir = "../save/dev_myeloid-Jul24-22-06-07"   
    save_dir = "../save/dev_myeloid-Jul24-15-14-57"          
    with open(save_dir + "/celltype_to_label.json",  "r") as f:
        cell_type_map = json.load(f)
        f.close()
    predict(save_dir, cell_type_map, test_batch_list=[0, 3, 4], modeldict_name="best_model_batch_4.pt")
    # predict(save_dir, cell_type_map, test_batch_list=[6, 7, 8], modeldict_name="best_model_batch_5.pt")
    # predict_testall(save_dir, cell_type_map, test_batch_list=[6, 7, 8])
