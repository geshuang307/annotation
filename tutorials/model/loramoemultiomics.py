...
from loramoeencoder import TransformerEncoder, TransformerEncoderLayer
...
class MultiOmicTransformerModel(nn.Module):
    def __init__(
        ...
        pre_norm: bool = False,
        use_mod: bool = False,
        use_multimod: bool = False,
        ntokens_mod: Optional[int] = None,
        vocab_mod: Optional[Any] = None,
        num_multimod_labels: Optional[int] = None,
        ...
        self.use_multimod = use_multimod
        self.num_multimod_labels = num_multimod_labels
        ...
        if use_multimod:
            self.multimod_encoder = BatchLabelEncoder(
                num_multimod_labels, d_model
            )
        ...
        if use_fast_transformer:
            ...
            encoder_layers = TransformerEncoderLayer(
                d_model, nhead, d_hid, dropout, batch_first=True, moe_experts=self.num_experts, adapter_dim=self.adapter_dim
            )
            self.transformer_encoder = TransformerEncoder(encoder_layers, nlayers)
        ...
        # self.cls_decoder = DecoderBank(d_model, n_cls, self.device)
        if do_mvc:
        ...
        if do_dab:
            ...
            # self.grad_reverse_discriminator = AdversarialDiscriminator(
            #     d_model,
            #     n_cls=self.num_multimod_labels,
            #     reverse_grad=True,
            # )
    ...
    def _encode(
        ...
        src = self.encoder(src)  # (batch, seq_len, embsize)  # torch.Size([32, 2385, 512])
        ...
        output = self.transformer_encoder(
            total_embs, batch_id=batch_id, src_key_padding_mask=src_key_padding_mask
        )
    ...
    def forward(
        ...
        batch_id: Optional[Tensor] = None,
        multimod_labels: Optional[Tensor] = None,
        CLS: bool = False,
        CCE: bool = False,
        MVC: bool = False,
        ECS: bool = False,
        ...
        transformer_output = self._encode(
            src, values, src_key_padding_mask, batch_labels, batch_id
        )
        if self.use_batch_labels:
            batch_emb = self.batch_encoder(batch_labels)  # (batch, embsize)
        ...
        if self.use_multimod:
            multimod_emb = self.multimod_encoder(multimod_labels)

        output = {}

        if self.use_batch_labels and self.use_mod and not self.use_multimod:
            cat_0 = (
                batch_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
                + mod_emb
            )
        elif self.use_batch_labels and not self.use_mod and not self.use_multimod:
            cat_0 = batch_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
        elif self.use_mod and not self.use_batch_labels and not self.use_multimod:
            cat_0 = mod_emb 
        elif self.use_batch_labels and self.use_mod and self.use_multimod:
            cat_0 = (
                batch_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
                + mod_emb + multimod_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
            )
        elif not self.use_batch_labels and self.use_mod and self.use_multimod:
            cat_0 = mod_emb + multimod_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
        else:
            cat_0 = None
        ...
        if CLS:
            output["cls_output"] = self.cls_decoder(cell_emb, batch_id)  # (batch, n_cls)
        if CCE:
            cell1 = cell_emb
        ...
        if MVC:
            if self.use_batch_labels and self.use_mod and not self.use_multimod:
                ...
                cat_1 = batch_emb + self._get_cell_emb_from_layer(mod_emb) + multimod_emb
                cat_2 = (
                    batch_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
                    + mod_emb +
                    multimod_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
                )
            elif not self.use_batch_labels and self.use_mod and self.use_multimod:
                cat_1 = self._get_cell_emb_from_layer(mod_emb) + multimod_emb
                cat_2 = mod_emb + multimod_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1)
            else:
                cat_1 = None
                cat_2 = None
...
class ContinuousValueEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = torch.clamp(x, max=self.max_value)   # torch.Size([32, 2385, 1])
...
class ClsDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, batch_id: None) -> Tensor:
        """
        Args:
...
class DecoderBank(nn.Module):
    """
    A container that holds multiple decoders. Each batch corresponds to a different decoder.
    """

    def __init__(self, d_model: int, n_cls: int, device=None):
        super().__init__()
        self.d_model = d_model
        self.n_cls = n_cls
        self.decoders = nn.ModuleList([ClsDecoder(d_model, n_cls)])  # Start with one decoder
        self.active_id = 0  # Tracks the current active decoder
        self.device = device
    ...
    def add_new_decoder(self):
        """Add a new decoder when starting to train a new task/batch."""
        new_decoder = ClsDecoder(self.d_model, self.n_cls)
        if self.device is not None:
            new_decoder = new_decoder.to(self.device)
        self.decoders.append(new_decoder)
        self.active_id = self.num_decoders - 1
        # self.active_id = self.num_decoders

    def freeze_decoder(self, idx: int):
        """Freeze the parameters of the decoder at the specified index."""
        for param in self.decoders[idx].parameters():
            param.requires_grad = False

    def freeze_all_previous_decoders(self, batch_id: int):
        """Freeze all decoders before the current batch_id."""
        for i in range(batch_id):
            self.freeze_decoder(i)

    def set_active(self, batch_id: int):
        """Set the current decoder to be trained."""
        self.freeze_all_previous_decoders(batch_id)
        self.active_id = batch_id
        for i, dec in enumerate(self.decoders):
            trainable = bool(i == int(self.active_id.numpy()))
            for p in dec.parameters():
                p.requires_grad = trainable

    def forward(self, x: Tensor, batch_id: int):
        """
        Forward pass for all decoders. Returns average logits and current decoder logits.

        Args:
            x: Input tensor.
            batch_id: The ID of the decoder to be used in the current batch.

        Returns:
            logits_avg: Average of all decoders' logits (used for loss calculation).
            logits_curr: Logits of the current decoder.
        """
        # self.set_active(batch_id)  # Ensure only the current decoder is active

        # Get current decoder's logits (will be used to compute gradients)
        ...
        if batch_id is not None:
            ...
            #     prev_logits.append(F.softmax(self.decoders[i](x), dim=-1))
            # prev_mean = torch.mean(torch.stack(prev_logits, dim=0), dim=0) 
            # return prev_mean
            logits_curr = self.decoders[0](x)
        # Get the logits from the previous decoders (do not compute gradients)
        # prev_logits = []
        # for i in range(self.num_decoders):
        #     if bool(i == int(self.active_id.numpy())):
        #         continue
        #     with torch.no_grad():
        #         prev_logits.append(self.decoders[i](x))

        # # Average the logits from current and previous decoders
        # if len(prev_logits) > 0:
        #     prev_mean = torch.mean(torch.stack(prev_logits, dim=0), dim=0)
        #     logits_avg = (logits_curr + prev_mean) / 2.0
        # else:
        #     logits_avg = logits_curr  # No previous decoders yet
