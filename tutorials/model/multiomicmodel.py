...
class MultiOmicTransformerModel(nn.Module):
    def __init__(
        ...
        super().__init__()
                # 直接打印所有参数
        print(
            f"ntoken={ntoken}, d_model={d_model}, nhead={nhead}, d_hid={d_hid}, nlayers={nlayers}, "
            f"nlayers_cls={nlayers_cls}, n_cls={n_cls}, vocab={vocab}, dropout={dropout}, "
            f"pad_token={pad_token}, pad_value={pad_value}, do_mvc={do_mvc}, do_dab={do_dab}, "
            f"use_batch_labels={use_batch_labels}, num_batch_labels={num_batch_labels}, "
            f"domain_spec_batchnorm={domain_spec_batchnorm}, input_emb_style={input_emb_style}, "
            f"n_input_bins={n_input_bins}, cell_emb_style={cell_emb_style}, "
            f"mvc_decoder_style={mvc_decoder_style}, ecs_threshold={ecs_threshold}, "
            f"explicit_zero_prob={explicit_zero_prob}, use_fast_transformer={use_fast_transformer}, "
            f"fast_transformer_backend={fast_transformer_backend}, pre_norm={pre_norm}, "
            f"use_mod={use_mod}, use_multimod={use_multimod}, ntokens_mod={ntokens_mod}, "
            f"vocab_mod={vocab_mod}, num_multimod_labels={num_multimod_labels}"
        )
    ...
    def forward(
        ...
        )                                               # torch.Size([16, 1614, 512])
        if self.use_batch_labels:
            batch_emb = self.batch_encoder(batch_labels)  # (batch,) -> (batch, embsize) torch.Size([16, 512])

        if self.use_mod:
            mod_emb = self.mod_encoder(mod_types)         # torch.Size([16, 1614])->torch.Size([16, 1614, 512])
