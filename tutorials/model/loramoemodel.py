...
class TransformerModel(nn.Module):
    def __init__(
        ...
        if use_fast_transformer:                             # False
            if fast_transformer_backend == "linear":
                self.transformer_encoder = FastTransformerEncoderWrapper(
                    d_model, nhead, d_hid, nlayers, dropout
                )
            elif fast_transformer_backend == "flash":       # "flash"
            ...
            encoder_layers = TransformerEncoderLayer(
                d_model, nhead, d_hid, dropout, batch_first=True, adapter_dim=adapter_dim, moe_experts=moe_experts
            )
            self.transformer_encoder = TransformerEncoder(encoder_layers, nlayers)                #  True
    ...
    def _encode(
        ...
        if getattr(self, "dsbn", None) is not None:
            ...
            total_embs = self.dsbn(total_embs.permute(0, 2, 1), batch_label).permute(
                0, 2, 1
            )  # the batch norm always works on dim 1 torch.Size([64, 1201, 512])
    ...
    def forward(
        ...
        if ECS:
            ...
            cell_emb_normed = F.normalize(cell_emb, p=2, dim=1)              # 64,512
