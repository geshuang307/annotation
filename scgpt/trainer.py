...
import itertools
from tutorials.functions.balanced_sampler import stratified_sample
...
def prepare_data(
    ...
    train_multimod_labels=None,
    valid_multimod_labels=None,
    ...
    print_label_distribution(tensor_batch_labels_train.numpy(), "Before stratified")

    if input_gene_ids_train.shape[0]>=4000 and train_multimod_labels is None:
    ...
    elif input_gene_ids_train.shape[0]>=4000 and train_multimod_labels is not None:
        (input_gene_ids_train,
            input_values_train,
            target_values_train,
            tensor_batch_labels_train,
    ...
    print_label_distribution(tensor_batch_labels_train.numpy(), "After stratified")
    train_data_pt = {
...
def train(
    ...
    for batch, batch_data in enumerate(itertools.islice(loader, num_batches)):
        ...
        print("input_values contains NaN:", torch.isnan(input_values).any())
        print("input_values contains inf:", torch.isinf(input_values).any())
        print("target_values contains NaN:", torch.isnan(target_values).any())
        print("target_values contains inf:", torch.isinf(target_values).any())

        if config["task"] == "annotation":
            celltype_labels = batch_data["celltype_labels"].to(device)
        if config["task"] == "multiomic":
            mod_types = batch_data["mod_types"].to(device)  # (B, 1614)
        ...
        with torch.cuda.amp.autocast(enabled=config["amp"]):
            ...
            if config["GEP"]:
                ...
                print('output_dict["mlm_output"].shape', output_dict["mlm_output"].shape)
            ...
            if config["DAR"]:
                ...
                loss_dab = criterion_dab(output_dict["dab_output"], multimod_labels)
        ...
        total_zero_log_prob += (
            loss_zero_log_prob.item() if config["GEP"] and config["explicit_zero_prob"] else 0.0
        )
    ...
    if "cls" in include_types:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=config["amp"]):
            cell_embeddings = model.encode_batch(
                ...
                batch_size=config["batch_size"],
                batch_labels=torch.from_numpy(batch_ids).long()
                if config["DSBN"] and config["DAR"] and config["use_batch_labels"]
                else None,
                time_step=0,
                return_np=True,
            )
