...
class scGPTIntegrationEnv(gym.Env):
    ...
    def step(self, actions, episode):      
        ...
        for data in selected_batches:
            ...
            with torch.cuda.amp.autocast(enabled=config['amp']):
                ...
                logger.info('Base model loss:', loss, 'fisher_loss:', net_loss)
                # if index >= 10:
                #     break
                    ############## 梯度反缩放和梯度裁剪 ##############
                    # scaler.unscale_(optimizer)
                    ...
                    # scaler.step(optimizer)
                    # scaler.update()
        # print('end_loss:', loss, 'net_loss:', net_loss)
        ################## 测试代码 ##################
        # for name, param in self.model.named_parameters():
    ...
    def compute_reward_kl(self, cell_embeddings):
        ...
        if os.path.exists(save_dir / 't_1_cell_embeddings.pth'):    # 过去时刻的embeddings
...
class PPO:
    ...
    def update(self, cell_embeddings_all, states, actions, old_log_probs, rewards, dones,  values, next_state, episode):    
        # Convert to tensor
        states = torch.FloatTensor(states)            # maxstep, 169*11=1859 
        actions = torch.FloatTensor(actions)          # maxstep, 169
        old_log_probs = torch.FloatTensor(old_log_probs)     # ?
        cell_embeddings_all = torch.stack(cell_embeddings_all, dim = 0).float().to(device)  # maxstep, 11990,512
...
# ========== 3. 训练 CPPO 代理 ========== #
def train_ppo(scgpt_model, dataset_loader_init, dataset_loader, num_episodes=500):      # 500
    ...
    ppo = PPO(env, env.model, state_dim=env.state_dim, action_dim=env.action_dim, hidden_dim = env.hidden_dim, input_dim = len(valid_loader_init)+len(train_loader_init), \
              batch_size=config['batch_size'], n_epochs = 10) 
