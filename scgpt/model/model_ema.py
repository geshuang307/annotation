...
class MyTransformerModel(nn.Module):
    ...
    # def initial_impt(self, n_encoder_layer, fc1_size, fc2_size, fc3_size):
    ...
    #     fc3_impt = torch.zeros(n_encoder_layer, fc3_size)   # 1*30
    #     fc3_mask = torch.ones(n_encoder_layer, fc3_size)
    #     fc3_mask.requires_grad_(requires_grad=True)
    ...
    def forward(
        ...
        if self.explicit_zero_prob and do_sample:
            bernoulli = Bernoulli(probs=mlm_output["zero_probs"])          # probs 概率取 1， 1-probs 概率取 0
            output["mlm_output"] = bernoulli.sample() * mlm_output["pred"]
        else:
            output["mlm_output"] = mlm_output["pred"]            # 未经过sigmoid激活之前 (batch, seq_len)
        if self.explicit_zero_prob:
            output["mlm_zero_probs"] = mlm_output["zero_probs"]  # √  经过sigmoid激活之后，直接用这个代表当前值为0的概率

        cell_emb = self._get_cell_emb_from_layer(transformer_output, values)  # transformer_output输出后 经过新的网络层得到cell_emb torch.Size([64, 512])
        ...
        if MVC:
            mvc_output = self.mvc_decoder(           # 基于隐空间的embeddings解码
                cell_emb                             # torch.Size([64, 512])
                if not self.use_batch_labels
            ...
            if self.explicit_zero_prob and do_sample:
                bernoulli = Bernoulli(probs=mvc_output["zero_probs"])           # "zero_probs"：模型预测某个基因表达值为 0（可能是 dropout）的概率。
                output["mvc_output"] = bernoulli.sample() * mvc_output["pred"]
            else:
                output["mvc_output"] = mvc_output["pred"]  # (batch, seq_len) √
            if self.explicit_zero_prob:
                output["mvc_zero_probs"] = mvc_output["zero_probs"]       # (batch, seq_len)
        if ECS:
            # Here using customized cosine similarity instead of F.cosine_similarity
            # to avoid the pytorch issue of similarity larger than 1.0, pytorch # 78064
            # normalize the embedding
            cell_emb_normed = F.normalize(cell_emb, p=2, dim=1)      # torch.Size([32, 512])
            cos_sim = torch.mm(cell_emb_normed, cell_emb_normed.t())  # torch.Size([32, 32]) (batch, batch)
            ...
            output["loss_ecs"] = torch.mean(1 - (cos_sim - self.ecs_threshold) ** 2)   # 计算余弦相似度与阈值之间的均方误差 (MSE) 阈值在0.8
...
class ExprDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, fc1_mask=None, fc2_mask=None) -> Dict[str, Tensor]:  # torch.Size([64, 1201, 1024])
        ...
        if fc1_mask is None:
            pred_value = self.fc1(x)          
        else:
            pred_value = self.fc1(x) * fc1_mask # torch.Size([32, 1201, 512])
        ...
        if self.return_representation:
            ...
            zero_probs = torch.sigmoid(zero_logits)            # 预测每个值 输出为0的概率
...
class MVCDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.arch_style in ["inner product", "inner product, detach"]:  # 'inner product'
            ...
            pred_value = torch.bmm(self.W(query_vecs), cell_emb).squeeze(2) # torch.Size([32, 1201, dim_in=1024])*torch.Size([32, 1024, 1])-》64, 1201 基因的embeddings和cell_embeddings相乘，每个基因对于每个细胞的重要性
            if not self.explicit_zero_prob:
                return dict(pred=pred_value)
...
def load_pretrained(
    ...
    fine_tune_params: Mapping[str, torch.Tensor],
    strict: bool = False,
    ...
    """
    pretrained_params = {k: v.detach().cpu() for k, v in pretrained_params.items()}
    fine_tune_params = {k: v.detach().cpu() for k, v in fine_tune_params.items()}
...
##################################
    if prefix is not None and len(prefix) > 0:
        ...
        model_dict.update(pretrained_params)    # 只加载了权重相同的那部分权重
    
    model_dict_fine_tune = model.state_dict()
    if strict:
        if verbose:
            for k, v in fine_tune_params.items():
                print(f"Loading parameter {k} with shape {v.shape}")
        model_dict_fine_tune.update(fine_tune_params)
        # model.load_state_dict(model_dict_fine_tune)
    else:
        if verbose:
            for k, v in fine_tune_params.items():
                if k in model_dict_fine_tune and v.shape == model_dict_fine_tune[k].shape:
                    print(f"Loading parameter {k} with shape {v.shape}")
        fine_tune_params = {
            k: v
            for k, v in fine_tune_params.items()
    ...
    assert set(model_dict.keys()) == set(model_dict_fine_tune.keys())
    alpha = 0.5
    theta = {
    key: (1-alpha) * model_dict[key] + alpha * model_dict_fine_tune[key]
...
def train(model: nn.Module, loader: DataLoader) -> None:
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config['amp']):
            ...
            output_dict["mlm_output"] = output_dict["mlm_output"].squeeze()           # 0.1343 ~ 0.3977
            ...
            if explicit_zero_prob:
                ...
                ###################
                mask = mask.float()
                bernoulli = torch.distributions.Bernoulli(probs=output_dict["mlm_zero_probs"])
                masked_log_probs = bernoulli.log_prob((target_values > 0).float()) * mask
                ###################
...
def tokenize_batch(
    ...
    for i in range(len(data)):
        ...
        if append_cls:
            genes = np.insert(genes, 0, cls_id)          # <cls> for the first gene
            values = np.insert(values, 0, 0)             # 0.0 for <cls>          
            if mod_type is not None:
...
if __name__=='__main__':
    ...
    seed=42,
    dataset_name="pancreas", # Dataset name
    ...
    # experiment_name = "fine_tune_onpbmc_ema"
    experiment_name = "fine_tune_onpancreas_ema"
    ...
    if hyperparameter_defaults['load_model'] is not None:  # load_model="/workspace/geshuang/code/scGPT/save/scGPT_human"
        ...
        # model_dir1 = Path("/workspace/geshuang/code/scGPT/tutorials/save/dev_PBMC_10K-Feb19-19-34")
        model_dir1 = Path("/workspace/geshuang/code/scGPT/tutorials/save/dev_pancreas-Feb25-17-11")
        print('model_dir', model_dir)
        model_config_file = model_dir / "args.json"
        model_file = model_dir / "best_model.pt"
        model_file1 = model_dir1 / "best_model.pt"
    ...
    if hyperparameter_defaults['load_model'] is not None:
        load_pretrained(model, torch.load(model_file), torch.load(model_file1), verbose=False)
