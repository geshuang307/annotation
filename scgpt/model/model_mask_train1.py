...
def train(model: nn.Module, loader: DataLoader) -> None:
    ...
    if epoch == 1:
        ...
        fc1_impt /= tot_tokens     # 对全部的样本都很重要的参数
        fc2_impt /= tot_tokens

        # Normalize the importance scores
        fc1_impt = impt_norm(fc1_impt)   # 归一化 1,30
        fc2_impt = impt_norm(fc2_impt) 
        
        print(fc1_impt.shape)
        print(fc2_impt.shape)
        ...
        #     )
        ######### I0 mask #########
        fc1_impt0 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc1_impt0.pt')
        fc2_impt0 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc2_impt0.pt')
        # fc1_mask0 = (1 - fc1_impt0[0]).to(device)     # 重要性高的units反而要被保留，因此mask是1-重要性
        # fc2_mask0 = (1 - fc2_impt0[0]).to(device)

        emax_fc1 = emax(fc1_impt, fc1_impt0)
        emax_fc2 = emax(fc2_impt, fc2_impt0)
        # fc1_mask1 = (1 - emax_fc1)     # 重要性高的units反而要被保留，因此mask是1-重要性
        # fc2_mask1 = (1 - emax_fc2)
        print('emax_fc1', emax_fc1)
        print('emax_fc2', emax_fc2)
        torch.save(emax_fc1, 'save/dev_PBMC_10K-Feb24-09-37/fc1_impt<=1.pt')   # 保存重要性 I(t<=1)
        torch.save(emax_fc2, 'save/dev_PBMC_10K-Feb24-09-37/fc2_impt<=1.pt')
        print('train epoch:1')
        ...
        for batch, batch_data in enumerate(loader):
            ...
            fc1_impt0 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc1_impt0.pt')
            fc2_impt0 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc2_impt0.pt')
            fc1_mask1 = (1 - fc1_impt0).to(device)     
            fc2_mask1 = (1 - fc2_impt0).to(device)
            with torch.cuda.amp.autocast(enabled=config['amp']):
                model.decoder.return_representation = False
                output_dict = model(
                    ...
                    MVC=config['GEPC'],
                    ECS=config['ecs_thres'] > 0,
                    fc1_mask = 1-fc1_mask1,
                    fc2_mask = 1-fc2_mask1,
        ...
        ######### soft-mask the network #########
            ...
            model.decoder.fc1.weight.grad *= fc1_mask1.transpose(0, 1)  # (1024, 512) * (512, 1)
            model.decoder.fc1.bias.grad *= fc1_mask1.squeeze()

            model.decoder.fc2.weight.grad *= fc2_mask1
            model.decoder.fc2.bias.grad *= fc2_mask1.squeeze()

            model.decoder.fc4.weight.grad *= fc1_mask1.transpose(0, 1)
            model.decoder.fc4.bias.grad *= fc1_mask1.squeeze()

            model.decoder.fc5.weight.grad *= fc2_mask1
            model.decoder.fc5.bias.grad *= fc2_mask1.squeeze()
