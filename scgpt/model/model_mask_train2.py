    ...
    def __init__(
        ...
        if do_dab:
            self.grad_reverse_discriminator = AdversarialDiscriminator(
                d_model,
                n_cls=num_batch_labels,         # batch 0 or 1
                reverse_grad=True,
            )
    ...
    def forward(
        ...
        if epoch == 1:
            ...
            if not self.use_batch_labels
                ...
                dim=2, 
            ), fc1_mask = fc1_mask, fc2_mask = fc2_mask
            # else transformer_output + batch_emb.unsqueeze(1),
        )
        else:
            mlm_output = self.decoder(
                ...
                if not self.use_batch_labels
                    ...
                    dim=2, 
                ), fc1_mask = None, fc2_mask = None
                # else transformer_output + batch_emb.unsqueeze(1),
            )
            transformer_output_pos = self._encode(
            src, values, src_key_padding_mask, batch_labels
        ) 

            mlm_output_pos = self.decoder(
                transformer_output_pos
                if not self.use_batch_labels
                else torch.cat(
                    [
                        transformer_output_pos,
                        batch_emb.unsqueeze(1).repeat(1, transformer_output_pos.shape[1], 1),
                    ],
                    dim=2, 
                ), fc1_mask = None, fc2_mask = None
                # else transformer_output + batch_emb.unsqueeze(1),
        ...
        # kd_loss = DistillKL(1)
        if output_contrast:
            output['pred_representation'] = mlm_output['pred_representation']
            output['zero_representation'] = mlm_output['zero_representation']
            output['pred_representation_pos'] = mlm_output_pos['pred_representation']
            # output['zero_representation_pos'] = mlm_output_pos['zero_representation']
            output['pred_representation_neg'] = mlm_output_neg['pred_representation']
...
class ExprDecoder(nn.Module):
    def __init__(
        ...
        #     nn.Linear(d_model, 1),
        # )
        self.fc1 = nn.Linear(d_in, d_model)      # First linear layer (1024,512)
        self.act1 = nn.LeakyReLU()               # First LeakyReLU activation
        self.fc2 = nn.Linear(d_model, d_model)   # Second linear layer (512, 512)
    ...
    def forward(self, x: Tensor, fc1_mask=None, fc2_mask=None) -> Dict[str, Tensor]:  # torch.Size([64, 1201, 1024])
        ...
        pred_representation = None
        zero_representation = None
        if fc1_mask is None:
        ...
        pred_representation = pred_value
        ...
        zero_representation = zero_logits
        zero_logits = self.fc6(zero_logits)
        zero_probs = torch.sigmoid(zero_logits)
        
        # return dict(pred=pred_value, zero_probs=zero_probs, pred_value1=pred_value1, zero_logits1=zero_logits1)
        return dict(pred=pred_value, zero_probs=zero_probs, pred_representation=pred_representation, zero_representation=zero_representation)
...
def train(model: nn.Module, loader: DataLoader) -> None:
    ...
    if epoch == 1:
        n_encoder_layer, fc1_size, fc2_size = 1, 512, 512
        fc1_impt2, fc1_mask2, fc2_impt2, fc2_mask2, tot_tokens = initial_impt(n_encoder_layer, fc1_size, fc2_size)
        for batch, batch_data in enumerate(loader):
            ...
            with torch.cuda.amp.autocast(enabled=config['amp']):
                ...
                #     ECS=config['ecs_thres'] > 0,
                # )
                # output_dict['pred_value1'] = output_dict['pred_value1'].squeeze()
                # contrastive_loss = contrastive_loss(ofull, ofull_pos, output_dict['pred_value1']*)
            ...
            model.zero_grad()
            scaler.scale(loss).backward()      # 全部损失前向传播
            fc1_impt2 += fc1_mask2.grad.clone().detach()
            fc2_impt2 += fc2_mask2.grad.clone().detach()
       
            tot_tokens += output_dict["mlm_output"].shape[0]
        
        fc1_impt2 /= tot_tokens     # 对全部的样本都很重要的参数
        fc2_impt2 /= tot_tokens

        # Normalize the importance scores
        fc1_impt2 = impt_norm(fc1_impt2)   # 归一化 1,30
        fc2_impt2 = impt_norm(fc2_impt2) 
        
        # print(fc1_impt2.shape)
        # print(fc2_impt2.shape)
        ...
        # print(f'fc1_impt usage: {(fc1_impt.sum() / fc1_impt.numel()).item()}')  # 平均重要性 0.59
        # print(f'fc2_impt usage: {(fc2_impt.sum() / fc2_impt.numel()).item()}')  # 平均重要性 0.54

        ######### I<=1 mask #########
        fc1_impt1 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc1_impt<=1.pt').detach().cpu()
        fc2_impt1 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc2_impt<=1.pt').detach().cpu()
        # fc1_mask0 = (1 - fc1_impt0[0]).to(device)     # 重要性高的units反而要被保留，因此mask是1-重要性
        # fc2_mask0 = (1 - fc2_impt0[0]).to(device)
        fc1_impt2 = fc1_impt2.detach().cpu()
        fc2_impt2 = fc2_impt2.detach().cpu()
        emax_fc1 = emax(fc1_impt1, fc1_impt2)
        emax_fc2 = emax(fc2_impt1, fc2_impt2)
        # fc1_mask1 = (1 - emax_fc1)     # 重要性高的units反而要被保留，因此mask是1-重要性
        # fc2_mask1 = (1 - emax_fc2)
        print('emax_fc1', emax_fc1)
        print('emax_fc2', emax_fc2)
        torch.save(emax_fc1, 'save/dev_PBMC_10K-Feb24-09-37/fc1_impt<=2.pt')   # 保存重要性 I(t<=1)
        torch.save(emax_fc2, 'save/dev_PBMC_10K-Feb24-09-37/fc2_impt<=2.pt')
        ...
        for batch, batch_data in enumerate(loader):
            ...
            fc1_impt1 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc1_impt<=1.pt').detach().cpu()
            fc2_impt1 = torch.load('save/dev_PBMC_10K-Feb24-09-37/fc2_impt<=1.pt').detach().cpu()
            fc1_mask2 = (1 - fc1_impt1).to(device)     
            fc2_mask2 = (1 - fc2_impt1).to(device)
            with torch.cuda.amp.autocast(enabled=config['amp']):
                model.decoder.return_representation = False
                output_dict = model(
                    ...
                    MVC=config['GEPC'],
                    ECS=config['ecs_thres'] > 0,
                    fc1_mask = 1-fc1_mask2,
                    fc2_mask = 1-fc2_mask2,
                    output_contrast=True,
                    epoch=epoch,
                )
                ofull = output_dict["pred_representation"]
                ofull_pos = output_dict['pred_representation_pos']
                oprev = output_dict["pred_representation_neg"]
                # output_dict['pred_value1'] = output_dict['pred_value1'].squeeze()
                con_loss = contrastive_loss(ofull, ofull_pos, oprev)
                ...
                if output_dict["dab_output"].ndimension() != 2:
                    ...
                    output_dict["dab_output"] = output_dict["dab_output"].unsqueeze(0)
                loss_dab = criterion_dab(output_dict["dab_output"], batch_labels)            # batch_loss
                loss = loss + config['dab_weight'] * loss_dab + con_loss           # total_loss      
                metrics_to_log.update({"train/dab": loss_dab.item()})
        ...
        ######### soft-mask the network #########
            ...
            model.decoder.fc1.weight.grad *= fc1_mask2.transpose(0, 1)  # (1024, 512) * (512, 1)
            model.decoder.fc1.bias.grad *= fc1_mask2.squeeze()

            model.decoder.fc2.weight.grad *= fc2_mask2
            model.decoder.fc2.bias.grad *= fc2_mask2.squeeze()

            model.decoder.fc4.weight.grad *= fc1_mask2.transpose(0, 1)
            model.decoder.fc4.bias.grad *= fc1_mask2.squeeze()

            model.decoder.fc5.weight.grad *= fc2_mask2
            model.decoder.fc5.bias.grad *= fc2_mask2.squeeze()
...
def load_data_and_model(hyperparameter_defaults,):
    ...
    return adata, adata_sorted, embsize, nhead, d_hid, nlayers, vocab, num_batch_types, model_file, logger, save_dir, \
        tokenized_train, tokenized_valid, train_batch_labels, valid_batch_labels, input_layer_key,gene_ids
...
if __name__=='__main__':
    ...
    hyperparameter_defaults, run, config = param(dataset_name, load_model, weight_dir)
    ...
    input_layer_key,gene_ids = load_data_and_model(hyperparameter_defaults, )
