...
class scGPTIntegrationEnv(gym.Env):
    ...
    def compute_reward(self):
        ...
        if os.path.exists('/workspace/geshuang/code/scGPT/scgpt/memory/t_1_cell_embeddings.pth'):
            ...
            kl_embeddings_t0 = kl(self.t_cell_embeddings, self.t0_cell_embeddings)
            reward = avg_cluster - 0.1 * kl_embeddings_t 
...
class CPPOAgent(nn.Module):
    ...
    def actor_critic(self, states):
        value_list_mean, log_probs_mean_list, value_list, log_probs_list, entropy_list, entropy_mean_list = [], [], [], [], [], []
        for state in states:
            for name, param in self.base_model.named_parameters():
                state_ndarray = state[1][name]
                if state_ndarray.shape == (512,):
                    print("The tensor shape is [512].")
                    state_tensor = torch.tensor(state_ndarray, dtype=torch.float32).unsqueeze(0)  # (1, state_dim)
                    action_prob, value = self.forward(state_tensor)
                    dist = Bernoulli(action_prob)           # 基于概率分布的随机采样
                    action = dist.sample()
                    log_prob = dist.log_prob(action)        # 计算对数概率 logP(A=a)=alogp+(1−a)log(1−p)
            ...
            value_list_mean.append(torch.mean(torch.stack(value_list)))
            log_probs_mean_list.append(torch.mean(torch.stack(log_probs_list)))
            entropy_mean_list.append(torch.mean(torch.stack(entropy_list)))

        return value_list_mean, log_probs_mean_list, entropy_mean_list
...
class PPO:
    def __init__(
        ...
        self.actor_critic = CPPOAgent.actor_critic
    ...
    def update(self, states, actions, old_log_probs, rewards, dones, next_state):    
        ...
        with torch.no_grad():
            # _, _, values = self.actor_critic(states)                                   # 当前状态输入到actor model和critic(预训练好的打分)model 得到当前输出（actor）和（value)
            # _, _, next_value = self.actor_critic(torch.FloatTensor([next_state]))  
                
            state_value_list_mean, state_log_probs_mean_list, _  = agent.get_action(states)   # 获取全部层动作和对数概率的list
            next_state_value_list_mean, next_state_log_probs_mean_list, _ = agent.get_action(next_state)
        # Compute advantages and returns
        values = torch.stack([t.squeeze() for t in state_value_list_mean])
        next_value = torch.stack([t.squeeze() for t in next_state_value_list_mean])
        ...
        for _ in range(self.n_epochs):
            ...
            state_value_list_mean, state_log_probs_mean_list, entropy_mean_list  = agent.get_action(states) 

            curr_log_probs = torch.FloatTensor(state_log_probs_mean_list)
            entropy = entropy_mean_list.mean()
            ...
            critic_loss = 0.5 * ((returns - values.squeeze()) ** 2).mean()       # 打分损失
...
# ========== 3. 训练 CPPO 代理 ========== #
def train_ppo(scgpt_model, dataset_loader_init, dataset_loader, num_episodes=2):      # 500
    ...
    ppo = PPO(state_dim=env.state_dim, action_dim=env.action_dim,batch_size=config['batch_size'])
    ...
    for episode in range(num_episodes):
        ...
        for t in range(maxsteps):
            ...
            log_probs.append(torch.mean(torch.stack([t.squeeze() for t in log_probs_list])).detach().numpy())
...
def plot_results(all_rewards, all_values, all_returns,  policy_loss_list, value_loss_list, entropy_loss_list):
    rewards_ = [reward.cpu().numpy() for reward in all_rewards]
    values_ = [value.detach().cpu().numpy() for value in all_values]
    returns_ = [returns.cpu().numpy() for returns in all_returns]
...
if __name__ == "__main__":
    ...
    scaler = torch.cuda.amp.GradScaler(enabled=config['amp'])  
    agent, policy_loss_list, value_loss_list, entropy_loss_list, all_rewards, all_values, all_returns = train_ppo(model, train_loader_init, train_loader)
    plot_results(all_rewards, all_values, all_returns, policy_loss_list, value_loss_list, entropy_loss_list)
