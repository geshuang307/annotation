...
# from scgpt.model.dap import ADPT_Block
...
class ADPT_Block(nn.Module):
    def __init__(self, ):
        ...
        # self.attention_norm = LayerNorm(self.hidden_size, eps=1e-6)  # config.hidden_size 512
        # self.ffn_norm = LayerNorm(self.hidden_size, eps=1e-6)        # config.hidden_size 512
        # self.ffn = Mlp(config)
        ...
        self.dap_downsample = nn.Linear(197, 3)               # dap_config.NUM_DAP_TOKENS
        nn.init.zeros_(self.dap_downsample.weight)
        nn.init.zeros_(self.dap_downsample.bias)
        self.dap_film = nn.Linear(64, 512 * 2)      # dap_config.TASK_EMB 64, config.hidden_size 512
        self.dap_norm = LayerNorm(512, eps=1e-6)       # config.hidden_size

    def forward(self, x, task_id_estimated_emb=None, layer_index=None, cfg=None):
        if x.shape[1] == 1201: # first layer   196 + 1(token length)
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        if input_emb_style == "continuous":                           # True
        ...
        if do_dab:                  # domain adversarial batch
        ...
        if use_dap:
            if num_domains is None:
                raise ValueError("num_domains must be provided when use_dap is True")
            if hidden_size is None:
                hidden_size = d_model
            self.adpt_block = ADPT_Block(
    ...
    def _encode(
        ...
        if self.use_dap:
            if batch_labels is None:
                raise ValueError("batch_labels must be provided when use_dap is True")
            total_embs = self.adpt_block(total_embs, batch_labels)
        ########################
        output = self.transformer_encoder(
            total_embs, src_key_padding_mask=src_key_padding_mask
        )
        return output  # (batch, seq_len, embsize) torch.Size([64, 1201, 512])
    ...
    def encode_batch(
        ...
        """
        N = src.size(0)                                # number of samples N = 16382
        ...
        outputs = array_func(shape, dtype=float32_)    # (16382, 512)
        ...
        for i in trange(0, N, batch_size):
            ...
            output = raw_output.detach()      # torch.Size([64, 1201, 512])
            if output_to_cpu:
                output = output.cpu()
            if return_np:
                output = output.numpy()
            if time_step is not None:         # time_step = 0
                output = output[:, time_step, :]     # (64, 512) 
...
class GeneEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.embedding(x)  # (batch, seq_len, embsize) torch.Size([64, 1201, 512])
        x = self.enc_norm(x)   # torch.Size([64, 1201, 512])
        return x
