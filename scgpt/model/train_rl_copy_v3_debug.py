...
#         # import scib
#         # import scib_metrics
#         import scib_metrics.benchmark as sbm
...
#         #     clisi_=False,
#         # )
#         bio_conservation = sbm.BioConservation(
#             isolated_labels=True,  # 计算孤立标签相关指标
#             nmi_ari_cluster_labels_leiden=True,  
#             nmi_ari_cluster_labels_kmeans=True,  # 计算 KMeans 聚类的 NMI 和 ARI
#             silhouette_label=True,  # 计算 silhouette 分数
#             clisi_knn=True  # 计算 cLISI 指标
#         )

#         # 选择批次校正指标（可以设为空，仅计算生物保真度）
#         batch_correction = sbm.BatchCorrection(
#             graph_connectivity=True,  # 计算图连通性
#             ilisi_knn = True,
#             kbet_per_label=True, 
...
class scGPTIntegrationEnv(gym.Env):
    ...
    def step(self, actions, episode):      
        ...
        num_batches = 1                         # 这里面选择的batch数目 100
        ...
        for data in selected_batches:
            ...
            with torch.cuda.amp.autocast(enabled=config['amp']):
                ...
                print('loss:', loss, 'net_loss:', net_loss)
                # scaler.scale(loss).backward()
                # print("Scaled Loss:", scaler.scale(loss).item())
                # print('loss:', loss)
                # if index >= 10:
    ...
    def reset(self):
        ...
        fisher_grad_path = '/workspace/geshuang/code/scGPT/scgpt/memory_v3/rl_fisher_optpar_grad_init_' + dataset_name +'v2.pth'
    ...
    def init(self, loader):
        ...
        self.gradient_features = np.stack(self.gradient_features)     # 169 * 11 的特征向量, 169层，每层11个特征点
        torch.save({'fisher': self.fisher, 'optpar': self.optpar, 'grad': self.gradient_features}, '/workspace/geshuang/code/scGPT/scgpt/memory_v3/rl_fisher_optpar_grad_init_' + dataset_name +'v2.pth')
    ...
    def save_init_embeddings(self):
        if os.path.exists('/workspace/geshuang/code/scGPT/scgpt/memory_v3/t0_cell_embeddings.pth'):
            cell_embeddings = torch.load('/workspace/geshuang/code/scGPT/scgpt/memory_v3/t0_cell_embeddings.pth')
        ...
        torch.save(cell_embeddings, '/workspace/geshuang/code/scGPT/scgpt/memory_v3/t0_cell_embeddings.pth')
        return cell_embeddings
    
    def get_init_embeddings(self):
        return torch.load('/workspace/geshuang/code/scGPT/scgpt/memory_v3/t0_cell_embeddings.pth')
...
def train_ppo(scgpt_model, dataset_loader_init, dataset_loader, num_episodes=1):      # 500
    ...
    for episode in range(num_episodes):
        ...
        for t in range(maxsteps):                 # 每个episode中，base model训练maxsteps次,每次又采样了多个batch的数据
            ...
            print('episode:',episode , 'step:', t, 'reward_kl',reward_kl)
    ...
    torch.save(agent.state_dict(), '/workspace/geshuang/code/scGPT/scgpt/memory_v3/ppo_agent.pth')
...
def plot_results(all_rewards, all_values, all_returns, all_actions, actor_loss_list, critic_loss_list, entropy_loss_list):
    ...
    plt.savefig('/workspace/geshuang/code/scGPT/scgpt/memory_v3/training_progress.png')
    plt.close()
    plt.figure(figsize=(5, 5))
    ...
    plt.savefig('/workspace/geshuang/code/scGPT/scgpt/memory_v3/loss.png')
    plt.close()
    results = {}
    ...
    # 保存字典
    with open("/workspace/geshuang/code/scGPT/scgpt/memory_v3/results.pkl", "wb") as f:
