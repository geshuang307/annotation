    ...
    def from_file(cls, file_path: Union[Path, str]) -> Self:
        ...
        if file_path.suffix == ".pkl":
            ...
            with file_path.open("r") as f:
                ...
                return cls.from_dict(token2idx)   # token-to-index 映射，并通过 cls.from_dict(token2idx) 将字典传递给 GeneVocab 构造函数来初始化
        else:
            raise ValueError(
...
def tokenize_batch(
    ...
    for i in range(len(data)):
        ...
        if include_zero_gene:
            ...
            idx = np.nonzero(row)[0]  # 682个非零值的索引
            values = row[idx]         # 
            genes = gene_ids[idx]
            if mod_type is not None:
                mod_types = mod_type[idx]
        if append_cls:
            genes = np.insert(genes, 0, cls_id)   # (683)
    ...
    return tokenized_data          # 只包含非零值的基因ID和表达值
...
def pad_batch(
    ...
    # print('################ batch ################', batch)
    if len(batch) == 0:
        max_ori_len = 0  # 或者 return None，看你具体需求
    else:
        max_ori_len = max(len(batch[i][0]) for i in range(len(batch)))
...
def tokenize_and_pad_batch(
    ...
    # print("tokenized_data length:", len(tokenized_data))
    # print("data:", data)
