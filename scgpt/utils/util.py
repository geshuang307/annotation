...
def set_seed(seed):
    ...
    torch.cuda.manual_seed(seed)                # 当前 GPU 上的随机数
    torch.cuda.manual_seed_all(seed)            # 所有 GPU 上的随机数
    os.environ['PYTHONHASHSEED'] = str(seed)    # 固定 hash 种子（可选）

    torch.backends.cudnn.deterministic = True   # 确保结果可复现
    torch.backends.cudnn.benchmark = False      # 禁用自动优化算法选择（为可复现性牺牲速度）
    # if n_gpu > 0:
